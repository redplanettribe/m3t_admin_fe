...
async function fetchOrganizersChatMessages(
  eventId: string,
  cursor?: string
  ...
  return apiClient.get<ChatMessagesListResponse>(
    `/events/${eventId}/chat/organizers/messages?${params.toString()}`
  )
}
...
export function useEventOrganizersChatMessages(
  eventId: string | null,
  enabled = true
) {
  const query = useInfiniteQuery({
    queryKey: queryKeys.events.chatOrganizers(eventId ?? ""),
    queryFn: ({ pageParam }) =>
      fetchOrganizersChatMessages(eventId!, pageParam as string | undefined),
